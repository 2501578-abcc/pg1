// フェーズ3: 記録を格納する配列を定義
let records = [];
let filteredRecords = []; // フィルタリング後の記録
...
function updateStatistics() {
    if (records.length === 0) {
        ...
        document.getElementById('max-mood').textContent = '-';
        document.getElementById('min-mood').textContent = '-';
        document.getElementById('mood-distribution').innerHTML = '<p style="color: #999;">データがありません</p>';
        return;
    }
    ...
    const variance = records.reduce((acc, r) => acc + Math.pow(r.mood - mean, 2), 0) / records.length;
    const stddev = Math.sqrt(variance);
    document.getElementById('stddev-mood').textContent = stddev.toFixed(2);
...
// レコードIDを指定して編集モーダルを開く（index.html?editId=... 用）
...
function displayRecords() {
    ...
    // 空の場合はCSSの:empty::before疑似要素でメッセージを表示するため、
    // ここでは何も表示しない（リストが空なら自動的にCSSでメッセージが表示される）
